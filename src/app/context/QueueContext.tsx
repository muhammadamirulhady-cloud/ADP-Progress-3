...
interface QueueContextValue {
  ...
  // Citizen actions
  issueTicket: (serviceKey: ServiceKey) => Promise<QueueTicket>;
...
export function QueueProvider({ children }: { children: ReactNode }) {
  ...
  // Issue a ticket via the backend (queue.php). Falls back to local
  // generation if the API is unreachable, so the kiosk never breaks.
  const issueTicket = useCallback(async (serviceKey: ServiceKey): Promise<QueueTicket> => {
    const service = SERVICES.find(s => s.key === serviceKey)!;

    try {
      const res = await fetch('http://localhost:8000/queue.php', {
        method: 'POST',
      ...
      const newTicket: QueueTicket = {
        ...
        issuedAt: new Date(t.issued_at).getTime(),
        status: 'waiting',
      ...
      // Backend unreachable — fall back to local ticket generation
      console.warn('queue.php unreachable, generating ticket locally:', err);
